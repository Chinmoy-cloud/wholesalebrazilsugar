function saveData(){
  ...
  $.ajax({
    type: "POST",
    url: BASE_URL +'admin/brand-save-data',
    data: formData,
    ...
    success:function(result){
      ...
      if(data.status == 1){
        ...
        toastr.success(data.msg, 'Saved', {timeOut: 1000,progressBar:true,onHidden:function() { window.location.href = BASE_URL+'admin/brands'; }   });
      } else {
...
function searchFilter(page_num,loadingType) {
  ...
  $.ajax({
    type: 'POST',
    url: BASE_URL+'admin/brand-search-data/'+page_num,
...
function statusChange(id,status){
  ...
  $.ajax({
    type: 'POST',
    url: BASE_URL+'admin/change-status-brand',
...
$(function () {
  ...
  var param = url.split('/');
  if( param[param.length - 1] && param[param.length - 1] == 'brands' ){
    searchFilter('',1);
...
$('#startEnd').on('cancel.daterangepicker', function(ev, picker) {
  ...
  $(".cancelButton").click(function(){
    window.location.href=BASE_URL+"admin/brands";
  });
